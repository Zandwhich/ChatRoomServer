...
- **Controller** — the server core. Opens a `ServerSocket`, accepts client connections in a loop, reads each client's initial handshake (JSON with a `name` field), wraps the connection in a `Participant`, and manages broadcasting. All outgoing messages are JSON built with `json_simple.JSONObject`. Colour support in messages is partially implemented (code exists but is commented out behind TODOs).
...
- **Server → Client named message:** `{"name": {"text": "<username>"}, "message": {"text": "<text>"}}`
- **Server → Client system message:** `{"message": {"text": "<text>"}}`
